...
#
# Build:
#   podman build -t eaglesnest-os:stable .
#
# Inspect (fast - no VM needed):
#   podman run --rm eaglesnest-os:stable rpm -qa | grep <package>
#   podman run --rm eaglesnest-os:stable ls /usr/lib/systemd/system/ | grep <service>
#   podman run --rm -it eaglesnest-os:stable bash
#
# Test (slow - builds VM):
#   just build-qcow2
#   just run-vm-qcow2
...
# Base Image
###############################################################################
FROM ghcr.io/ublue-os/bluefin:stable@sha256:742dfe52ce1deeacbb7b44b87366037ed7f1fb4e47707a3eb6198730678c07cf
